...
{
    public class PCPAPartyAddressDT : IPartyAddressUIDT
    {
        public List<PartyAddressUI> GetPartAddress(SourceSystemBaseEntity sourceSystemEntity, bool isPrimaryAddress, bool includeCurrentUser)
        {
            ...
            if (((PLAutoSourceSystemEntity)sourceSystemEntity).PolicyData != null)
                return GetPartyAddress(partyList, ((PLAutoSourceSystemEntity)sourceSystemEntity).PolicyData.Policy.PolicyPeriod.ListedParties.Select(x => x.Party).ToList()).OrderBy(x => x.DisplayName).ToList();
            return partyList;
        }
            ...
            foreach (var party in contacts)
            {
                try
                {
                    ...
                    if (party.Person != null)
                        ...
                        {
                            EmailAddress = party.Person.EmailAddresses?.Select(x => x.UnparsedEmailAddress).ToList()
                        };
